...
public class RegisterLineService {
    ...
    public LineMissionResponse registerCommand(String userId, String message) {
        ...
        List<Long> stationGroupIdList  = handleStationGroup(dto.getStationName());
     ...
     * @return
     */
    private List<Long> handleStationGroup(String stationName) {
//        stationService.find
        Long count = stationGroupRepository.countByStationName(stationName);
        if (count != 1) {
            ask();
        } else {
            stationGroupRepository.findByStationName(stationName);
        }
